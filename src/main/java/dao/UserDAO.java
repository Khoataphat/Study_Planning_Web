    ...
    public void markSetupDone(int userId) throws Exception {
        ...
        try (Connection con = DBUtil.getConnection(); PreparedStatement ps = con.prepareStatement(sql)) {
            ...
            ps.executeUpdate();
             ...
             System.err.println("Lỗi khi markSetupDone: " + e.getMessage());
            e.printStackTrace();
            throw e;
        }
    }
...
public int createOAuthUser(User u) throws Exception {
    ...
    // Thêm phương thức mới theo cách đơn giản
    public User getUserById(int userId) throws Exception {
        String sql = "SELECT user_id, username, email, is_first_login FROM users WHERE user_id = ?";
        try (Connection con = DBUtil.getConnection(); PreparedStatement ps = con.prepareStatement(sql)) {
            ps.setInt(1, userId);
            ResultSet rs = ps.executeQuery();
