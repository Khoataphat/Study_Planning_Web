...
public class User {
    ...
    // Thêm phương thức để chuyển Role enum sang String
    public String getRoleString() {
        return role != null ? role.name() : Role.STUDENT.name();
    }
    
    // Thêm phương thức để set Role từ String
    public void setRoleFromString(String roleStr) {
        try {
            this.role = Role.valueOf(roleStr.toUpperCase());
        } catch (IllegalArgumentException e) {
            this.role = Role.STUDENT; // Mặc định là STUDENT nếu không hợp lệ
        }
    }
    
    // Thêm phương thức kiểm tra nếu user là admin
    public boolean isAdmin() {
        return role == Role.ADMIN;
    }
    
    // Thêm phương thức kiểm tra nếu user là student
    public boolean isStudent() {
        return role == Role.STUDENT;
    }
    
    // Thêm phương thức kiểm tra nếu user đã đăng nhập lần đầu
    public boolean isFirstLogin() {
        return isFirstLogin == 1;
    }
    
    // Thêm phương thức kiểm tra nếu user đã hoàn thành setup
