...
public class TaskDAO {
    ...
    private void linkTaskToUser(int taskId, int userId, Task task) {
        try {
            ...
            try (Connection conn = DBUtil.getConnection(); PreparedStatement stmt = conn.prepareStatement(insertSql, Statement.RETURN_GENERATED_KEYS)) {
...
// ⭐️ HÀM MỚI: Chuyển đổi Calendar.DAY_OF_WEEK sang hệ thống của chúng ta
    ...
    private int findOrCreateCollection(int userId) throws SQLException {
        ...
        try (Connection conn = DBUtil.getConnection(); PreparedStatement stmt = conn.prepareStatement(findSql)) {
            stmt.setInt(1, userId);
            ResultSet rs = stmt.executeQuery();
            if (rs.next()) {
        ...
        try (Connection conn = DBUtil.getConnection(); PreparedStatement stmt = conn.prepareStatement(createSql, Statement.RETURN_GENERATED_KEYS)) {
            stmt.setInt(1, userId);
            stmt.executeUpdate();
...
// Helper method: chuyển Timestamp sang day_of_week (1-7)
    ...
    public int getCompletedTaskCount(int userId) {
        String sql = "SELECT COUNT(*) as completed_count FROM tasks t "
                + "INNER JOIN user_schedule us ON t.task_id = us.task_id "
                + "INNER JOIN schedule_collection sc ON us.collection_id = sc.collection_id "
                + "WHERE sc.user_id = ? AND t.status = 'completed'";
    ...
    public List<Task> getTasksDueSoon(int userId, int days) {
        List<Task> tasks = new ArrayList<>();
        String sql = "SELECT t.* FROM tasks t "
                + "INNER JOIN user_schedule us ON t.task_id = us.task_id "
                + "INNER JOIN schedule_collection sc ON us.collection_id = sc.collection_id "
