...
CREATE TABLE `progress_snapshots` (
  ...
  `snapshot_type` enum('daily','weekly','monthly') COLLATE utf8mb4_unicode_ci NOT NULL,
  `snapshot_date` date NOT NULL,
  `total_study_minutes` int DEFAULT '0',
  `completed_tasks_count` int DEFAULT '0',
  `completed_subjects_count` int DEFAULT '0',
  `average_focus_score` decimal(3,2) DEFAULT NULL,
  ...
  UNIQUE KEY `unique_user_snapshot` (`user_id`,`snapshot_type`,`snapshot_date`),
  CONSTRAINT `progress_snapshots_ibfk_1` FOREIGN KEY (`user_id`) REFERENCES `users` (`user_id`) ON DELETE CASCADE
